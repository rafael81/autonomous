...
def test_normalize_roma_events_preserves_builtin_tool_names():
    rows = normalize_roma_events(
        ...
        raw_events=[
            ...
            {"type": "tool_call", "name": "glob_paths", "callId": "call-4", "args": {"pattern": "tests/test_*.py"}},
            {"type": "tool_result", "name": "glob_paths", "callId": "call-4", "output": "tests/test_cli.py"},
            {"type": "session_end", "ok": True},
        ],
    )

    tool_names = [row["payload"].get("tool_name") for row in rows if "tool_call" in row["event_type"]]
    assert tool_names == ["list_dir", "list_dir", "read_file", "read_file", "grep_text", "grep_text", "glob_paths", "glob_paths"]
